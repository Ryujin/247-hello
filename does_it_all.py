...
def map():
    ...
    lbl = Label(window, justify=LEFT, text="With the QP chat interface screen up,\nclick the 'New' tab at upper left above the blue bar", font=("Helvetica", 14)).pack(side='left')
    ...
    window.geometry('750x230+300+225')
    window.configure(background='pink')
    window.lift()
    lblPic2 = Label(window, image=pic2).pack(side="left")
    lbl = Label(window, justify=LEFT, text="Now, click just below the blue bar", font=("Helvetica", 14)).pack(side='left')
    with Listener(on_click=on_click) as listener:
        window.mainloop()
        listener.join()
    listener.stop()
    ...
    window.geometry('1000x330+300+225')
